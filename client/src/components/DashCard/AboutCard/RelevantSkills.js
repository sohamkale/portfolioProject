...
import RelevantSkillsCard from "./RelevantSkillsCard.js";
import "./RelevantSkills.css";
const RelevantSkills = (props) => {
...
//------------------------------------------------------------------------------------------------------------------------------
    ...
    var refSoftSkills = db.ref(`${userUid}/About/RelevantSkills`); //reference to the softSkills child of about
//------------------------------------------------------------------------------------------------------------------------------
    const addSkill = () => {
        let skillObj = {
            'id': (uniqueId + 1).toString(),
            'relevantSkill': "programming skill",
            'percent': "competance %"
        }
        ...
        refAbout.child('RelevantSkills').child(uniqueId + 1).update(
            {
                'relevantSkill': "programming skill",
                'percent': "competance %"
            }
...
//------------------------------------------------------------------------------------------------------------------------------
    const deleteSkill = (e) => {
       ...
       //e is the button pressed so to get the id we need e.target.id
        ...
        refAbout.child('RelevantSkills').child(e.target.id).remove(); //remove the correct id from the database
